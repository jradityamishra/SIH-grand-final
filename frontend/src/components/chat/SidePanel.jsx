  ...
  const setNewgroupUser = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:8000/api/v1/auth/getuser?search=${search}`
      );
      console.log(data);
      setLoading(false);
  ...
  const handleDeleteGroup = async (index) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8000/api/v1/chat/delete/${index._id}`
      );
      console.error(response);

      setGroups((prevGroups) =>
        prevGroups.filter((group) => group._id !== index._id)
      );
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/api/v1/chat/getGroup/${userStudent._id}`
      );
          ...
          <div className="bg-white p-4 w-96">
            ...
            {selectedUsers.map((u) => (
              <span
                ...
                className="inline-flex items-center my-1 mx-1 px-2 py-1 me-2 text-sm font-medium text-gray-800 bg-gray-100 rounded dark:bg-gray-700 dark:text-gray-300"
              >
                {u.name}
                <button
                  type="button"
            ...
            {loading ? (
              ...
              searchResult?.map((user) => (
                <div
                  key={user._id}
                  className="bg-indigo-900 text-center py-4 lg:px-4 cursor-pointer"
                ...
                >
                  ...
                  >
                    <img
                      ...
                      src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTNq-fhMeQRIAFfcfgPFaQDO8yTQ_SOW1-6raA_0HgiiKDJTV0TkDiojPT98h40g8T4FAk&usqp=CAU"
                      alt="Bordered avatar"
                    />

                    <div className="flex flex-col ml-2">
                      <span className="font-semibold mr-2 text-left flex-auto mt-1 mb-1">
                        {user.name}
                      </span>
